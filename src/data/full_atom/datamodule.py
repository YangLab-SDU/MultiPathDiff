...
class FullAtomDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        self.val_gen_dataset = self.hparams.dataset.get("val_gen_dataset", None)
        if self.val_gen_dataset:
            self.val_gen_dataset.repr_loader = self.hparams.repr_loader

        self.test_gen_dataset = self.hparams.dataset.get("test_gen_dataset", None)
    ...
    def val_dataloader(self):
        ...
        if self.val_gen_dataset is not None:
    ...
    def test_dataloader(self):
        if self.test_gen_dataset is None:
            return None
...
class ClusterDistributedSampler(DistributedSampler):
    def __init__(self, csv_path, batch_size):
        ...
        for sublist in self.cluster_indices:
            total_padded_samples += int(np.ceil(len(sublist) / self.batch_size)) * self.batch_size

        total_batches = total_padded_samples // self.batch_size
    ...
    def __iter__(self):
        ...
        chunks = [batch_indices[i: i + self.batch_size] for i in range(0, len(batch_indices), self.batch_size)]

        my_chunks = chunks[self.rank:: self.num_replicas]

        indices = np.concatenate(my_chunks).tolist()
        return iter(indices)
    ...
    def reinit(self):
        self.epoch += 1
        print(f"Shuffling pure batches... seed {41 + self.epoch}")
        np.random.seed(41 + self.epoch)

        batch_chunks = []
        cluster_order = np.random.permutation(len(self.cluster_indices))

        for cluster_idx in cluster_order:
            sublist = self.cluster_indices[cluster_idx].copy()
            np.random.shuffle(sublist)

            for i in range(0, len(sublist), self.batch_size):
                chunk = sublist[i: i + self.batch_size]

                if len(chunk) < self.batch_size:
                    pad_size = self.batch_size - len(chunk)
                    pad_samples = np.random.choice(sublist, size=pad_size, replace=True)
                    chunk = np.concatenate([chunk, pad_samples])

                batch_chunks.append(chunk)

        np.random.shuffle(batch_chunks)
        batch_indices = np.concatenate(batch_chunks)
