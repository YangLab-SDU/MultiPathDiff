...
class GuidanceDataset(torch.utils.data.Dataset):
    ...
    def load_pdb(self, pdb_path):
        ...
        atom_coords = np.full((seqlen, rc.atom_type_num, 3), np.nan)

        for res_idx, residue in enumerate(residues):
            if residue.has_id('CA'):
                resname = residue.resname.strip()
                seqres += rc.restype_3to1.get(resname, "X")
            else:
                seqres += "X"
            ...
            for atom in residue:
                ...
                if atom_name in rc.atom_order:
                    ...
                    atom_coords[res_idx, atom_type_idx] = atom.coord
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        gt_force_0 = np.load(
            f"{self.data_dir}/{row.chain_name}/{row.chain_name}_Step_{row.sample_id}_opt_ca_force_clip.npy"
        )
        ...
        atom_coords, seqres = self.load_pdb(
            os.path.join(
                self.data_dir,
                f"{row.chain_name}",
                f"{row.chain_name}_Step_{row.sample_id}_opt.pdb",
            )
        )

        current_seq_len = len(seqres)
        ...
        t_one = torch.ones(current_seq_len)
