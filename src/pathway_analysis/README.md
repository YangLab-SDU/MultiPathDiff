...
probabilities to MultiPathDiff trajectories and cluster trajectories with similar
...
3. Cluster folding trajectories using residue folding times and contact
   formation times, then rank the clusters using the pathway probability.
...
| `pathway_probability.py` | Filters invalid MSTA files and assigns normalized pathway probabilities using predefined taxonomic-proximity tiers. |
| `pathways_cluster.py` | Extracts folding-time features, calculates pairwise pathway distances, clusters trajectories, and selects probability-guided cluster representatives. |
...
order. The probability assignment script writes this association as `path_index` in
`pathway_probability.csv`, and the clustering script matches `path_index=X` to
`sampleX`.
...
`pathway_probability.csv`. The script also writes
...
not used during probability assignment.
...
python pathways_cluster.py \
  ...
  --pathway_probability_csv /path/to/1AB7_A/pathway_cluster/pathway_probability.csv \
...
the final structures. Cluster probabilities are the sums of their member pathway
probabilities. The highest-probability member of each cluster is reported as its
representative.

The calculation is unchanged. These are normalized, taxonomy-based pathway
probabilities, not experimentally calibrated kinetic branching probabilities.
The input CSV must contain `path_index` and `probability`. The clustering option
is `--pathway_probability_csv`. Existing CSV files using the previous column
name must be regenerated or have that column renamed to `probability`.
The default output directory for probability assignment is
`<root>/<target>/pathway_probability` when `--output_dir` is omitted.

Cluster output columns include `path_probability`, `cluster_probability_sum`,
`cluster_probability_renormalized_valid`, `total_matched_path_probability`,
and `centroid_path_probability`. Structural feature and clustering filenames
remain unchanged because they do not represent probability tables alone.
