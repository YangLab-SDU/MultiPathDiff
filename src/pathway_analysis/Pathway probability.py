...
def tier_rank_power_scores(
    ...
    if total <= 0 or not math.isfinite(total):
        raise ValueError("The sum of tier scores is invalid. Check --probability_rank_power.")
...
def process_one_target(args, target: str) -> None:
    ...
    if len(valid_labels) == 0:
        raise ValueError(f"Target {target} has no valid temp_msa_X.msta files, so pathway scores cannot be assigned.")
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "CSV containing afdb_labels. If omitted, the script searches <target>/pathway_probability, "
            "<target>/pathway_score, and <target> for *with_label*.csv or *label*.csv."
        ),
    )
    parser.add_argument(
        "--msta_dir",
    ...
    parser.add_argument(
        "--output_dir",
        default=None,
        help="Output directory. Default: <root>/<target>/pathway_score.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0.35,
        help=(
            "Compatibility option retained from older commands. This script assigns taxonomy scores but does not cluster, "
    ...
    if not target:
        raise ValueError("Specify a target protein, for example: python 'Pathway probability.py' 1AB7_A")
