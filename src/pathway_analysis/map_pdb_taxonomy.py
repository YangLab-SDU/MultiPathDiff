...
def parse_pdb_chain_id(s):
    ...
    if m is None:
        raise ValueError(f"Cannot parse protein name/PDB_CHAIN: {s}")
    ...
    with open(input_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                print(f"[WARNING] Cannot parse this line; skipping: {s}")
...
def detect_accession_column(df):
    ...
    raise ValueError("Neither the uniprot_accessions nor uniprot_accession column was found in the DataFrame.")
...
def assign_labels_to_dataframe(df, acc_col, accession_to_label):
    ...
    for _, row in df.iterrows():
        ...
        if not accessions:
            # If the PDB entry cannot be mapped to a UniProt accession, assign label 4 (cellular_organisms) by default.
        ...
        if not matched_records:
            # If the UniProt accession is absent from the AFDB label file, assign label 4 (cellular_organisms) by default.
            ...
            # In the rare case where matching records exist but the label is empty, also assign label 4 by default.
            labels = DEFAULT_UNKNOWN_LABEL
...
def main():
    ...
    parser.add_argument(
        "proteins",
        nargs="*",
        help="Protein name or PDB_CHAIN supplied directly, for example 1AB7_A, 1AB7:A, 1AB7-A, '1AB7 A', or 1AB7",
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[],
        help="One or more protein names/PDB_CHAIN identifiers, for example -i 1AB7_A or -i 1AB7_A 1RIS_A",
    )

    parser.add_argument(
        "--input-list",
        default=None,
        help="Optional compatibility mode for an old list.txt input, with one PDB ID or PDB_CHAIN per line",
    )
    ...
    parser.add_argument(
        ...
        help="File mapping afdb_accession values to category labels, for example merged_target_h_with_label.clean.txt",
    )
    ...
    parser.add_argument(
        ...
        help="Final output CSV file; default: uniprot_mapping.with_label.csv",
    )
    ...
    parser.add_argument(
        ...
        help="Number of CPU processes used to scan the label file in parallel. Defaults to SLURM_CPUS_PER_TASK, or 1 if unset.",
    )

    parser.add_argument(
        "--no-uniprot-details",
        action="store_true",
        help="Output only UniProt accessions without querying entry, protein, or gene names; this can reduce runtime.",
    )
    ...
    if not items:
        raise SystemExit(
            "No valid input was provided. Examples:\n"
            "  python map_pdb_taxonomy.py 1AB7_A -l merged_target_h_with_label.clean.txt\n"
            "  python map_pdb_taxonomy.py -i 1AB7_A 1RIS_A -l merged_target_h_with_label.clean.txt\n"
            "  python map_pdb_taxonomy.py --input-list list.txt -l merged_target_h_with_label.clean.txt"
        )
    ...
    if df.empty:
        raise RuntimeError("No PDB-UniProt mapping records were generated. Check the input protein names.")
