...
# -*- coding: utf-8 -*-
"""
Cluster multiple PathDiffusion folding trajectories without predefining cluster number.
...
Input example:
    frames_dir/
        ...
        model_1_sample0.pdb
        ...
        model_299_sample0.pdb
        model_0_sample1.pdb
        ...

Each sampleX is treated as one folding pathway/trajectory.

This version does NOT require a native PDB. It uses the predicted final model(s),
normally model_299_sampleX.pdb, as references.

The clustering distance uses two terms by default:
    1) per-residue CA-distance folding time
       For each sample, every frame is globally aligned to that sample's own final
       structure, normally model_299_sampleX.pdb. A residue is called folded when
       its post-alignment CA distance to that final structure is <= cutoff for
       consecutive frames.

    2) contact formation time
       Reference contacts are defined from model_299 structures instead of native.
       By default, a consensus contact set is built across all sample final models.
       A contact is called formed when its CA-CA distance approaches the reference
       contact distance for consecutive frames.
...
Main outputs:
    ...
    cluster_summary.csv
    # contact_formation_time.csv and reference_contacts.csv are optional; use --save_contact_outputs

Notes:
    - Use --n_jobs to calculate sample-level features and pairwise distances with
      multiple CPU processes.
    - No --native argument is required. If --native is passed for compatibility,
      it is ignored.
"""
...
def compute_contact_formation_times(
    ...
    formed_score = thresholds[None, :] - contact_distances
...
def cluster_paths(
    ...
    the tree into a fixed number of clusters. This keeps the downstream score-based cluster sorting and centroid selection unchanged.
...
def load_pathway_score_csv(score_csv, sample_ids=None):
    """
    Load pathway scores from pathway_score.csv.

    This replaces the old pathway_probability.csv logic:
        - No original probability is read.
        - No rank-power rescoring is performed here.
        - The score column is used directly for cluster sorting and centroid selection.

    Expected columns:
        path_index: numeric sample id; path_index=0 corresponds to sample0.
        score: pathway score produced by the taxonomy score script.

    Rows with empty path_index or empty/non-finite score are ignored.

    Returns:
        path_score_by_sample: dict[str, float], e.g. {"sample0": 0.18}
        used_df: DataFrame with sample and score columns for matched paths.
    """
    if score_csv is None or str(score_csv).strip() == "":
        return None, pd.DataFrame([])

    if not os.path.exists(score_csv):
        raise FileNotFoundError(f"--pathway_score_csv not found: {score_csv}")

    df = pd.read_csv(score_csv)
    if "path_index" not in df.columns:
        raise ValueError(f"{score_csv} must contain a 'path_index' column")
    if "score" not in df.columns:
        raise ValueError(f"{score_csv} must contain a 'score' column")
    ...
    records = []
    score_by_sample = defaultdict(float)
    ...
    for _, row in df.iterrows():
        ...
        score_by_sample[sample_name] += score
    ...
    if len(used_df) == 0:
        print(f"WARNING: no valid pathway score rows were loaded from {score_csv}")
        return {}, pd.DataFrame([])

    # If a path_index appears more than once, sum scores.
    grouped = (
        used_df.groupby(["path_index", "sample"], as_index=False)
        .agg(path_score=("score", "sum"))
        .sort_values(["path_score", "path_index"], ascending=[False, True])
        .reset_index(drop=True)
    )
    grouped["score_rank"] = np.arange(1, len(grouped) + 1, dtype=int)

    path_score_by_sample = {
        str(row["sample"]): float(row["path_score"])
        for _, row in grouped.iterrows()
    }

    return path_score_by_sample, grouped


def reorder_labels_by_cluster_score(labels, sample_ids, path_score_by_sample=None):
    """
    Reindex non-noise cluster labels so cluster 0 has the largest total score.
    If no pathway scores are provided, use cluster size as the sorting score.
    """
    labels = np.asarray(labels, dtype=int)
    ...
    for lab in sorted(set(labels.tolist())):
        ...
        if path_score_by_sample is None:
            score = float(len(idx))
        else:
            score = float(sum(path_score_by_sample.get(m, 0.0) for m in members))
        cluster_scores.append((lab, score, len(idx)))

    # Larger score first; if tied, larger cluster size first; if still tied, old label order.
    cluster_scores = sorted(cluster_scores, key=lambda x: (-x[1], -x[2], x[0]))
    mapping = {old_lab: new_lab for new_lab, (old_lab, _, _) in enumerate(cluster_scores)}
...
def summarize_clusters(sample_ids, labels, D, path_score_by_sample=None):
    """
    Build pathway_clusters.csv and cluster_summary.csv.

    If path_score_by_sample is provided:
        - cluster score is the sum of all member path scores.
        - centroid_sample is the member path with the largest score.
          If several members have the same score, the one with the smallest
          within-cluster distance sum is selected.

    If path_score_by_sample is not provided:
        - cluster score is cluster occupancy by path count.
    ...
    labels = np.asarray(labels, dtype=int)
    n = len(sample_ids)
    use_path_score = path_score_by_sample is not None

    cluster_records = []
    summary_records = []

    if use_path_score:
        total_matched_score = float(sum(path_score_by_sample.get(s, 0.0) for s in sample_ids))
    else:
        total_matched_score = np.nan
    ...
    for lab in sorted(set(labels.tolist())):
        ...
        member_scores = np.array(
            [path_score_by_sample.get(m, 0.0) for m in members],
            dtype=np.float64,
        ) if use_path_score else np.full(len(idx), np.nan, dtype=np.float64)

        if use_path_score:
            cluster_score = float(np.nansum(member_scores))
            max_score = float(np.nanmax(member_scores)) if len(member_scores) > 0 else np.nan

            candidate_local = np.where(np.isclose(member_scores, max_score, rtol=1e-12, atol=1e-15))[0]
            if len(candidate_local) == 1:
                centroid_local = int(candidate_local[0])
            else:
                # Tie-break: among equal-score paths, use the one most central by distance.
            ...
            selection_rule = "max_path_score"
            cluster_score_renormalized_valid = (
                cluster_score / total_matched_score
                if np.isfinite(total_matched_score) and total_matched_score > 0
                else np.nan
            )
        else:
            cluster_score = n_path_fraction
            ...
            cluster_score_renormalized_valid = np.nan
        ...
        summary_records.append({
            ...
            "cluster_score_sum": cluster_score,
            "cluster_score_renormalized_valid": cluster_score_renormalized_valid,
            "total_matched_path_score": total_matched_score,
            "centroid_sample": centroid_sample,
            "centroid_path_score": max_score if use_path_score else np.nan,
        ...
        for local_pos, i in enumerate(idx):
            sample = sample_ids[i]
            path_score = float(member_scores[local_pos]) if use_path_score else np.nan
    ...
    cluster_df = pd.DataFrame(cluster_records).sort_values(
        ["is_noise", "cluster", "path_score", "sample"],
        ascending=[True, True, False, True],
    )
    summary_df = pd.DataFrame(summary_records).sort_values(
        ["is_noise", "cluster_score_sum", "n_paths"],
...
def run(args):
    ...
    path_score_by_sample, pathway_score_used_df = load_pathway_score_csv(
        args.pathway_score_csv,
        sample_ids=sample_ids,
    )
    if path_score_by_sample is not None:
        matched_sum = sum(path_score_by_sample.get(f"sample{sid}", 0.0) for sid in sample_ids)
        missing_samples = [f"sample{sid}" for sid in sample_ids if f"sample{sid}" not in path_score_by_sample]
        print(f"Loaded pathway scores from: {args.pathway_score_csv}")
        print("The score column is used directly for cluster sorting and centroid selection.")
        print(f"Matched pathway score sum over loaded samples: {matched_sum:.10f}")
        if missing_samples:
            print("WARNING: no score found for these samples; their path_score will be 0: " + ",".join(missing_samples))
    ...
    labels = reorder_labels_by_cluster_score(
        labels=labels,
        sample_ids=sample_names,
        path_score_by_sample=path_score_by_sample,
    )
    ...
    print("Top clusters:")
    if len(top3_df) > 0:
        print(top3_df[["cluster", "n_paths", "cluster_score_sum", "centroid_sample", "members"]].to_string(index=False))
    else:
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            ...
            "Pathway ranking/centroid selection can be guided directly by pathway_score.csv."
        )
    )
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Optional pathway_score.csv. The script reads path_index and score directly. "
            "path_index is matched to sample id, e.g. path_index=0 -> sample0. "
            "After clustering, cluster score is the sum of member path scores, and "
            "the cluster centroid/medoid is the member path with the largest score."
        ),
    )
